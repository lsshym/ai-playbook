...
| :--- | :--- | :--- | :--- | :--- |
| `ProductCard` | PROPS | High | `ProductCard.tsx` | Consolidate 8 related scalar product props into `product={product}` and update the child props type when the semantics match. |
| `ProductFormState` | INLINE_TYPE | Medium | `./types.ts` (New) | Extract the inline `useState` shape into a colocated type because it names reusable form state. |
...
| `OrderPanel` | ORDER | Low | `OrderPanel.tsx` | Reorder hooks, derived values, handlers, and JSX without changing runtime behavior. |
