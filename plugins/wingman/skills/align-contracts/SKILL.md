...
4. **Classify the gap**:
   ...
   - Semantic mismatch: names or values imply different meaning (`status` vs `checkoutType`). Do not resolve this by renaming, deleting, or narrowing the consumer contract to the provider concept.
...
- Renaming or deleting the consumer contract to hide semantic uncertainty, such as changing `toCheckoutType` into `toCheckoutStatus`.
