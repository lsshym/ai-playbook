...
这个目录只放 `memory-setup`、`memory-load`、`memory-sync` 的评估用例和 fixture。运行、快照、HTML 报告由 `runner.mjs` 调用公共脚本生成，并参考 `skill-evals/align-contracts` 的输出结构。
...
2. 在 `runner.mjs` 中添加对应 fixture builder。
