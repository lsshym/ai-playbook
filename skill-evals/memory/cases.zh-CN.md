...
| MEM-SYNC-04 | memory-sync | user-override, skip-update | 已完成一个有意义的 checkout bugfix，但用户明确说“这个不用记忆 / skip update / 不更新”。 | 用户覆盖优先。重点检查即使改动有价值，也完全不修改 `.wingman/memory`；不能写 active log，不能写 domain truth，不能归档旧日志。最终说明应简短表达已按用户要求跳过 memory 更新。 |
